...
class HofstadterFermions(CouplingMPOModel):
    ...
    def init_lattice(self, model_params):
        bc_MPS = model_params.get('bc_MPS', self.default['bc_MPS'])
        order = model_params.get('order', self.default['order'])
        site = self.init_sites(model_params)
        Lx = model_params.get('Lx', self.default['Ly'])
        Ly = model_params.get('Ly', self.default['Lx'])
    ...
    def init_terms(self, model_params):
        Lx = self.lat.shape[0]
        Ly = self.lat.shape[1]
        phi_ext = model_params.get('phi_ext', self.default['phi_ext'])
        mu = np.asarray(model_params.get('mu', self.default['mu']))
        v = np.asarray(model_params.get('v', self.default['v']))
